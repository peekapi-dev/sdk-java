...
public class PeekApiFilter implements Filter {
    ...
    private String resolveConsumerId(HttpServletRequest req) {
        ...
        return ConsumerIdentifier.identify(
                req.getHeader("x-api-key"), req.getHeader("Authorization"));
    }
    ...
    private static class CountingPrintWriter extends PrintWriter {
        ...
        public void write(int c) {
            super.write(c);
            byteCount += Character.toString((char) c).getBytes(java.nio.charset.StandardCharsets.UTF_8).length;
        }

        @Override
        public void write(char[] buf, int off, int len) {
            super.write(buf, off, len);
            byteCount += new String(buf, off, len).getBytes(java.nio.charset.StandardCharsets.UTF_8).length;
        }

        @Override
        public void write(String s, int off, int len) {
            super.write(s, off, len);
            byteCount += s.substring(off, off + len).getBytes(java.nio.charset.StandardCharsets.UTF_8).length;
        }
