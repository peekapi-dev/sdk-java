...
class SsrfProtectionTest {
    ...
    void ipv4MappedIpv6IsPrivate() throws UnknownHostException {
        ...
        assertTrue(
                SsrfProtection.isPrivateAddress(
                        InetAddress.getByName("::ffff:10.0.0.1")));
        // ::ffff:192.168.1.1
        assertTrue(
                SsrfProtection.isPrivateAddress(
                        InetAddress.getByName("::ffff:192.168.1.1")));
    }
    ...
    void ipv4MappedIpv6PublicIsAllowed() throws UnknownHostException {
        assertFalse(
                SsrfProtection.isPrivateAddress(
                        InetAddress.getByName("::ffff:8.8.8.8")));
    }
    ...
    void validateHostRejectsPrivateIp() {
        assertThrows(
                ...
                () -> SsrfProtection.validateHost("10.0.0.1"));
        assertThrows(
                IllegalArgumentException.class,
                () -> SsrfProtection.validateHost("192.168.1.1"));
        assertThrows(
                IllegalArgumentException.class,
                () -> SsrfProtection.validateHost("172.16.0.1"));
    }
}
